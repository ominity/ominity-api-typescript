...
export function match<T, E>(
  ...
  return async function matchFunc(
    ...
    [
      result: Result<T, E | OminityDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new OminityDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new OminityDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
