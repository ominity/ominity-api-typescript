...
export function currenciesGet(
    client: ClientSDK,
    request: operations.GetCurrencyRequest,
    options?: RequestOptions,
): Promise<
    Result<
        operations.GetCurrencyResponse,
        | OminityError
        | ConnectionError
        | InvalidRequestError
...
async function $do(
    ...
    >
> {
    const path = pathToFunc("/commerce/currencies/{code}")({
        code: request.code,
    });
    ...
    const [result] = await M.match<
        ...
        | RequestTimeoutError
    >(
        M.json(200, operations.GetCurrencyResponse$inboundSchema),
