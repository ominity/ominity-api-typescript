...
export function shippingZonesGet(
    client: ClientSDK,
    request: operations.GetShippingZoneRequest,
    options?: RequestOptions,
): Promise<
    Result<
        operations.GetShippingZoneResponse,
        | OminityError
        | ConnectionError
        | InvalidRequestError
...
async function $do(
    ...
    >
> {
    const path = pathToFunc("/commerce/shipping-zones/{id}")({
        id: request.id,
    });
    ...
    const [result] = await M.match<
        ...
        | RequestTimeoutError
    >(
        M.json(200, operations.GetShippingZoneResponse$inboundSchema),
