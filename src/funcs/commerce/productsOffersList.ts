...
export function productsOffersList(
    client: ClientSDK,
    request: operations.ListProductOffersRequest,
    options?: RequestOptions,
): Promise<
    Result<
        operations.ListProductOffersResponse,
        | OminityError
        | ConnectionError
        | InvalidRequestError
...
async function $do(
    ...
    >
> {
    const path = pathToFunc("/commerce/products/{id}/offers")({
        id: request.id,
    });
    ...
    const [result] = await M.match<
        ...
        | RequestTimeoutError
    >(
        M.json(200, operations.ListProductOffersResponse$inboundSchema),
