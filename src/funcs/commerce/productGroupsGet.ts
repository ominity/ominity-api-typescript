...
export function productGroupsGet(
    client: ClientSDK,
    request: operations.GetProductGroupRequest,
    options?: RequestOptions,
): Promise<
    Result<
        operations.GetProductGroupResponse,
        | OminityError
        | ConnectionError
        | InvalidRequestError
...
async function $do(
    ...
    >
> {
    const path = pathToFunc("/commerce/product-groups/{id}")({
        id: request.id,
    });
    ...
    const [result] = await M.match<
        ...
        | RequestTimeoutError
    >(
        M.json(200, operations.GetProductGroupResponse$inboundSchema),
