...
export function reviewsGet(
    client: ClientSDK,
    request: operations.GetReviewRequest,
    options?: RequestOptions,
): Promise<
    Result<
        operations.GetReviewResponse,
        | OminityError
        | ConnectionError
        | InvalidRequestError
...
async function $do(
    ...
    >
> {
    const path = pathToFunc("/commerce/reviews/{id}")({
        id: request.id,
    });
    ...
    const [result] = await M.match<
        ...
        | RequestTimeoutError
    >(
        M.json(200, operations.GetReviewResponse$inboundSchema),
