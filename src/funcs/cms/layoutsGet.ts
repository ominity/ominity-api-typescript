...
export function layoutsGet(
    client: ClientSDK,
    request: operations.GetLayoutRequest,
    options?: RequestOptions,
): APIPromise<
    Result<
        operations.GetLayoutResponse,
...
> {
    ...
    const path = `/cms/layouts/${safeId}`;

    const query = encodeFormQuery({}); // No query params for layout get in current spec
    ...
    const [result] = await M.match<
        ...
        | SDKValidationError
    >(
        M.json(200, operations.GetLayoutResponse$inboundSchema, { ctype: "application/hal+json" }),
