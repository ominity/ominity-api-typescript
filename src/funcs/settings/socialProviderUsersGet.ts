 ...
 */
export async function socialProviderUsersGet(
    client: ClientSDK,
    request: operations.GetSocialProviderUserRequest,
    options?: RequestOptions,
): Promise<
    Result<
        operations.GetSocialProviderUserResponse,
        | OminityError
        | SDKValidationError
        | UnexpectedClientError
        | InvalidRequestError
    ...
    >
> {
    const input: operations.GetSocialProviderUserRequest = request;
    ...
    const context = {
        operationID: "getSocialProviderUser",
        oAuth2Scopes: [],
    ...
    >(
        M.json(
            200,
            operations.GetSocialProviderUserResponse$inboundSchema,
        ),
        M.fail(
